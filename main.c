...
======================================================================
================
* Project: Student Attendance & Marks Manager (CLI)
* Author: Pallab Sarangi
* Description: A tool to manage student records, attendance, and
marks.
* Demonstrates structs, arrays, file handling, and modular
programming.
*
======================================================================
================
...
float marks; // Final marks (e.g., out of 100)
} Student;
// --- GLOBAL VARIABLES ---
// We use a global array for simplicity in this introductory level.
Student class_list[MAX_STUDENTS];
int student_count = 0; // Tracks how many students are currently inthe list
// --- FUNCTION PROTOTYPES ---
// Declaring functions before main() lets the compiler know theyexist.
void addStudent();
void markAttendance();
void enterMarks();
void displayReport();
...
scanf("%d", &choice);
clearInputBuffer(); // Fixes skipping issues with scanf andstrings
switch(choice) {
...
// First line: Save the total count so we know how many to readlater
fprintf(file, "%d\n", student_count);
...
// Clear buffer after reading the integer, so fgets works for th ename below
fgetc(file);
for (int i = 0; i < student_count; i++) {
